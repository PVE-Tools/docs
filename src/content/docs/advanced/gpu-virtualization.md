...
:::caution[维护边界]
SR-IOV 的"稳定"源自于内核版本、headers、DKMS 驱动、kernel cmdline 参数、VF 自动生成机制这几项长期保持一致。请把它当作"需要去维护的驱动栈"而非"一次配置便永久稳定"来看待。
:::
...
:::caution[内核升级风险]
SR-IOV 极其依赖内核头文件。如果你升级了 PVE 内核，必须重新去运行脚本或者重新编译 DKMS 驱动，否则虚拟核显会全部消失！
:::
